...
def _find_gaps(
    ...
    engagement_validities = sorted(engagement_validities, key=lambda x: x["from"])
    ...
    if manager["validity"]["from"] < engagement_validities[0]["from"]:
        gap_end_date = engagement_validities[0]["from"] - datetime.timedelta(days=1)
        gaps.append(
            InvalidManagerPeriod(
                uuid=manager["uuid"],
                from_=manager["validity"]["from"],
    ...
    for i in range(len(engagement_validities) - 1):
        ...
        next_start_date = engagement_validities[i + 1]["from"]
    ...
    invalid_manager_gaps = [
        ...
        and (gap.to is None or gap.to >= manager["validity"]["from"])
    ]
