...
from terminate_managers_initialiser.query_to_find_empty_managers import get_empty_managers
from terminate_managers_initialiser.find_no_engagement_managers import extract_managers_with_no_persons_or_engagements
...
async def terminator_initialiser(gql_client: GraphQLClient):
    ...
    """
    print("Initialising search for managers with no engagements or persons associated with the role.")
    try:
        ...
        manager_uuids_with_no_engagements = extract_managers_with_no_persons_or_engagements(manager_objects)
        ...
        if manager_uuids_with_no_engagements:
            await terminate_existing_empty_manager_roles(gql_client, manager_uuids_with_no_engagements)
            print("Successfully terminated empty managers.")
            logger.info("Terminated empty manager(s) with uuid(s):", manager_uuids=manager_uuids_with_no_engagements)
            return None
