...
async def test_initiate_terminator():
    ...
    mo_get_managers_mock = AsyncMock(
        return_value={"objects":TEST_DATA_MANAGERS}
    )
...
async def test_initiate_terminator_tailing_engagements():
    ...
    # mocking
    mo_get_managers_mock = AsyncMock(
        return_value={"objects":test_data}
    )

    mo_terminate_manager_mock = AsyncMock(
        return_value=TerminateManagerManagerTerminate(uuid=test_data[0]["validities"][0]["uuid"])
    )
...
async def test_init_when_no_managers_found(
    ...
    # ACT
    await search_empty_manager_roles(mocked_mo_client)
...
@patch("manager_terminator.terminate_managers_init.init_manager_terminator.logger")
async def test_terminate_manager_roles(
    mock_events_logger,
...
@patch("manager_terminator.terminate_managers_init.init_manager_terminator.logger")
async def test_update_manager_roles_vacant(
    mock_events_logger,
    mock_extract_managers_uuid_and_end_date: MagicMock,
):
    """
    Tests if the update function is awaited.
    ...
    mock_extract_managers_uuid_and_end_date.return_value = list_of_updates

    # ACT
    await search_empty_manager_roles(mocked_mo_client)
    for termination_data in list_of_updates:
        manager_uuid = termination_data.get("uuid")  # type: ignore
        vacant_from = termination_data.get("termination_date")
        await mocked_mo_client.update_manager(manager_uuid, vacant_from)

        # ASSERT
        mocked_mo_client.update_manager.assert_awaited_with(
            manager_uuid, vacant_from
        )

    # ASSERT
    mock_events_logger.info.assert_any_call(
        "Updated empty manager(s) to vacant with uuid(s):",
        manager_uuids=list_of_updates,
    )
